...
class RappIndexer(object):
    ...
    __slots__ = ['raw_data_path', 'raw_data', 'package_whitelist', 'package_blacklist']

    def __init__(self, raw_data=None):
        self.raw_data_path = {}
        self.raw_data = {}
        self.package_whitelist = None
    ...
    def get_compatible_rapps(self, uri=rocon_uri.default_uri_string):
        '''
          ...
          :returns: a list of compatible rapps
          :rtype: [rocon_app_utilities.Rapp]

          :raises: RappAncestorconflictException: an exception when more than one implementation of an ancestor are found 
        '''
        ...
        for resource_name, rapp in compatible_rapps.items():
            resolved_rapp = self.get_rapp(resource_name)
            ancestor_name = resolved_rapp.ancestor_name

            if ancestor_name in used_ancestors:
                raise RappAncestorConflictException(resource_name, ancestor_name, used_ancestors[ancestor_name])
